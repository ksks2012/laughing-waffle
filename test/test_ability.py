...
def test_all_number_abilities():
    all_pass = []
    img_list = list(range(1, 12))

    for j in range(2, 11):
        img_list[j] = cv2.imread(f"./test_data/training/{j}_image.png", 0)

    print(len(RESULTS))
    for i in RESULTS:
        tmp = 0
        for j in range(2, 11):
            thres = thres_img2(img_list[j], i[0], i[1])
            NUMBER_CONFIG = "--psm 10 --oem 3 -c tessedit_char_whitelist=0123456789"
            try:          
                num = int(pytesseract.image_to_string(Image.fromarray(thres), lang='eng', config=NUMBER_CONFIG))
            except:
                break
            else:
                print(f"{i[0]}, {i[1]}, {j} == {num}")
                if num == j:
                    tmp += 1
                else:
                    break
